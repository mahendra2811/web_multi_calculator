...
import { getSchema } from "@/lib/calculators/schemas";
import { SchemaCalculator } from "@/components/calculator/SchemaCalculator";
import type { CalculatorMeta } from "@/types/calculator";

function CalculatorDispatcher({ meta }: { meta: CalculatorMeta }) {
...
export function CalculatorLoader({ meta }: { meta: CalculatorMeta }) {
  ...
  const schema = getSchema(meta.id);
  if (schema) {
    return <SchemaCalculator meta={meta} schema={schema} />;
  }
